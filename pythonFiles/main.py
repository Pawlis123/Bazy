...
def magic():
    try:
        ...
        for x in range(100):
            base = "INSERT INTO festivals (festivalstartingdate, festivalendingdate, festivalname) VALUES (%s, %s, %s)"
            randDays = random.randint(5, 100)
            randDate1 = datetime.now() + timedelta(days=randDays)
            randDays = random.randint(5, 100)
            randDate2 = randDate1 + timedelta(days=randDays)

            cursor.execute(base, (randDate1, randDate2, "festival" + str(x + result[0])))
            connection.commit()
        ...
        cursor.execute(count)
        eventsResult = cursor.fetchone()

        for x in range(2000):
            luck = random.randint(0, 1)
            if luck == 0:
                # base
                base = "INSERT INTO events (eventname, eventdate, eventtime, localization, organizer, festival) VALUES "

                base += "('eventname"
                base += str(x + eventsResult[0])

                base += "', '"
                randDays = random.randint(5, 300)
                base += str(datetime.now() + timedelta(days=randDays))
                ...
                cursor.execute(count)
                result = cursor.fetchone()
                help = "SELECT * FROM localizations WHERE localizationname='localizationname"
                help += str(random.randint(0, result[0] - 1))
                help += "'"
                cursor.execute(help)
                result = cursor.fetchone()
                ...
                cursor.execute(count)
                result = cursor.fetchone()
                help = "SELECT * FROM organizers WHERE organizername='organizerName"
                help += str(random.randint(0, result[0] - 1))
                help += "'"
                cursor.execute(help)
                result = cursor.fetchone()
                ...
                cursor.execute(count)
                result = cursor.fetchone()
                help = "SELECT * FROM festivals WHERE festivalname='festival"
                help += str(random.randint(0, result[0] - 1))
                help += "'"
                cursor.execute(help)
                result = cursor.fetchone()
                ...
                base = "INSERT INTO events (eventname, eventdate, eventtime, localization, organizer) VALUES "
        ...
        for x in range(700):
            while True:
                # base
                base = "INSERT INTO artistsevents (event, artist) VALUES "
                base += "("

                # append foreign key
                count = "SELECT COUNT(*) FROM events"
                cursor.execute(count)
                eventResult = cursor.fetchone()
                help = "SELECT * FROM events WHERE eventname='eventname"
                eventId = random.randint(0, eventResult[0] - 1)
                help += str(eventId)
                help += "'"
                cursor.execute(help)
                eResult = cursor.fetchone()
                base += str(eResult[0])
                base += ', '
                ...
                check = "SELECT COUNT(*) FROM artistsevents WHERE event="
                check += str(eResult[0])
                check += " AND artist="
                check += str(aResult[0])
